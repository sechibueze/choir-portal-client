...
const PostCard = ({ post, likePostByPostId, unlikePostByPostId }) => {
  const { _id, title, content, likes } = post;
  return (
    <Fragment>
      <div className="post-card py-2">
        <div className="post-content p-1">
          <p className="text text-primary"> {title && title} </p>
          <article className="post-content my-1">{content && content}</article>
          <div className="post-stats my-1">
            <span
              title="Message Acknowledged"
              onClick={() => likePostByPostId(_id)}
            ...
            <span
              ...
              onClick={() => unlikePostByPostId(_id)}
            ></span>
