...
import { ADD_NEW_MEMBER_SUCCESS, EDIT_MEMBER_SUCCESS, DELETE_MEMBER_SUCCESS, DELETE_MEMBER_FAIL, MEMBERS_UPLOAD_SUCCESS, TOGGLE_ADMIN_SUCCESS, TOGGLE_ADMIN_FAIL } from '../../_actions/types';
import 'react-data-table-component-extensions/dist/index.css';
import Spinner from '../../components/Spinner';


const Members = ({ 
    membersRequest, deleteMemberRequest, toggleAdminRequest,editMemberRequest,
    removedMember, members,  roleStatus, updatedMember,membersUpload,newMember,
    deleteMembers, toggleAdminRole,  getMembers, resetMemberData }) => {
    ...
    useEffect(getMembers, [newMember, updatedMember, removedMember,membersUpload, roleStatus])
    ...
    const clearRowSelection = () => {
        if( deleteMemberRequest === false || editMemberRequest === false || toggleAdminRequest === false) 
            setToggledClearRows(!toggledClearRows)
    }
    useEffect( clearRowSelection, [deleteMemberRequest, editMemberRequest, toggleAdminRequest]);
    ...
    // ------------------------
    const DeleteMembersButton = ({selection}) => <span className="context-action" onClick={() => handleDelete(selection.selectedRows)}> <span className="fas fa-user-minus"/>Delete</span>
    const ManageAuthButton = ({ selection }) => {
        return <span className="context-action" onClick={() => handleRoleUpdate(selection.selectedRows)}> <span className="fas fa-user-shield" />Update Role</span>
    }
    ...
    const TableContextComponents = ({ selection, ...props}) => {
        ...
        if (selection.selectedCount === 1) {
            ...
            return [tableOptions.editButton, tableOptions.manageAuthButton, tableOptions.deleteButton, ]
        }
